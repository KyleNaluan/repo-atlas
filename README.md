...
Four Flow adapters trace an execution story ([#35](https://github.com/KyleNaluan/repo-atlas/issues/35)).
...
The fourth runs the other way - it starts at a durable record the subject both writes and reads through its own SQL, and fans out to the independent derivations that read it, each labelled by a code identifier and the literal SQL predicate its read writes.
They are registered separately, because "this subject runs no Spring", "this subject ships no runnable main", "this subject's frontend calls nothing it serves" and "this subject stores no record it derives from" are different findings and one adapter may not answer for another; an adapter whose framework is absent reports `not_applicable` with its reason rather than running empty.
