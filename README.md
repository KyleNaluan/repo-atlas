...
Seven Flow adapters trace an execution story ([#35](https://github.com/KyleNaluan/repo-atlas/issues/35)).
...
They are registered separately, because "this subject runs no Spring", "this subject ships no runnable main", "this subject's frontend calls nothing it serves", "this subject stores no record it derives from", "this subject runs no batch work", "this subject consumes no messages" and "this subject ships no unit files" are different findings and one adapter may not answer for another; an adapter whose framework is absent reports `not_applicable` with its reason rather than running empty.
