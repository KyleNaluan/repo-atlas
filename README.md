...
What `test/run/parity.test.ts` still pins as open, rather than letting a green suite imply the artifact has caught up entirely: no boundary candidate survives the floor on this subject (a probe-coverage gap, not a scoring one), and the produced graph sits at 18 nodes against the reference's 33.
