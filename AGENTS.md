...
**Issue comments are the first decision source and not the only one (#55).** That source is empty on three of the four subjects this engine targets (0/16, 0/13, 0/39 measured 2026-08-27), so `src/harvest/records.ts` reads the tree as a second one. A span of markdown is a record only where the SUBJECT'S OWN DECLARATION says so - four families, listed in README's "The two decision sources": a decision-record directory (`ADR_DIRECTORIES`, shared with #6's density signal), a filename that names the file one, or a decision-naming HEADING in a project-memory file or any other document.

It is a heading match and never a body match, for #28's reason, and the vocabulary deliberately over-admits: the writer owns admissibility, and an over-admitted section becomes an absent cut naming its reason. Tightening the matcher until it is never wrong would move acceptance into a regex.

**#4's memory-file line is amended, not lifted.** A memory file is still inadmissible as evidence ABOUT THE CODE - that guarantee is structural, since `implemented_by` is empty at write time and only `settleBuild` fills it from paths the gate located. What #55 admits is testimony about a DECISION, the same class as a resolution comment and at the same `attested` ceiling. Harvest's `sources[]` states both lines separately and `test/harvest/tree.test.ts` watches them.
...
`src/probes/flow/` is the shared machinery and `src/probes/library/flow-*.ts` are the ten registered adapters (#35 PRs 4-8, plus #52's three Python ones). One family per adapter, deliberately: "no Spring here", "no runnable main here", "no frontend calls anything here", "nothing here stores a record it derives from", "no batch work here", "nothing consumed here", "no unit files here", "no FastAPI here", "no runnable Python entry here" and "no declared LangGraph topology here" are ten different findings, which is what the `Probe.applies` hook exists for - a framework-level applicability answer the toolchain test cannot give.
...
`assets/tree-sitter-java.wasm` and `assets/tree-sitter-python.wasm` are vendored deliberately. The only npm package shipping prebuilt grammars bundles ~40 of them at 50 MB for two files of 430 KB and 476 KB, which is not a defensible npx footprint. `web-tree-sitter` is pinned to the ABI both grammars were built against - **they move together or not at all**, and since #52 that is a THREE-file change rather than two.

## The Python adapter (#52)

The design is closed on [#52](https://github.com/KyleNaluan/repo-atlas/issues/52) (D1-D5) over an investigation report; read the resolutions before touching a stage it owns.
It is a PORT, not an invention: `candidate.ts`, `retained`, `route.ts`, `sql.ts`, `rank/flow.ts` and the whole `FlowClaim` contract are reused verbatim, which is why `py-symbols.ts` fills the SAME `TypeSymbol`/`MethodSymbol` shapes rather than inventing its own - two candidate emitters would be two definitions of what a box is.

What Python fills those shapes with is its own, and is the part worth reading:

- **A module is a pseudo-type** and so is a class, and `bean` is true for both, because in Python a file and a class are exactly what the subject declares itself to have. That is what buys `candidate.ts`'s one-box-per-TYPE rule: three functions of one module the story passes through are one box. The declared-pipeline adapter is the exception - there a NODE is the component, because the topology declares nodes as its parts.
- **Name resolution is by IMPORT, never by a global simple-name map.** Java can ask "which subject type is called `Store`"; Python cannot, because the answer is whatever the importing file said. `py-module.ts` is the ONE definition of what a file is importable as (Python's own rule: the import name starts at the first ancestor that is not a package), shared by producer and gate for the reason `route.ts` shares "the same route" - resolution stays split.
- **Four receiver shapes and no fifth** (report 4.2): a module-qualified free-function call, `self.m` / `self._x.m` typed by the three `__init__` shapes, an annotated parameter/local/class-name, and a same-file `def`'s return annotation propagated one step. The `__init__` initialiser path is admissible here where Java's `var` is not, and the reason is the GATE's reach: `self._driver = BookSessionDriver(...)` is a literal assignment one regex finds in the same file.
- **`Any` and an unannotated `def` are NAMED stops, not silences.** That is risk R2 and the largest single gap measured: dsa's whole pipeline is unreachable because `build_graph` is annotated `-> Any`, and "this subject's execution path passes through a value it declines to type" is a true statement about the subject. An explicit `-> None` is Python's `void` and is foreign. `declaredReturn` (`py-symbols.ts`) keeps those three states apart; `MethodSymbol.returns` carries them.
- **An annotation is unwrapped only where the wrapper denotes the SAME OBJECT** (`Optional`, `type`). Reducing `list[DecisionRecord]` to its element made `records.append(...)` read as a missing `DecisionRecord.append` - a hole in a story that has none. "One element of this" is a separate question (`annotationElement`) asked only by a `for` target, and a call on such an element is `unresolved_dispatch:` because v1 closes no set through a collection.
- **One closure rule**, per D3: a keyed registry that is a module-level dict literal (`py-dispatch.ts`). Type closure is provably wrong by one on dsa (11 `Tool` subclasses, 10 in the injected list) and value closure crosses two files against the single-file gate rule. The union type alias IS Python's `sealed` and is recorded rather than used - an alias says what the set is, not that a call reaches all of it.
- **D1 keeps framework-callback entries out** and emits `framework_callback_unestablished:` per declared callback, from `flow-python-console-entry` because that adapter owns "how does this subject's code get started". A `Protocol`/`ABC` base is excluded: that is a contract the SUBJECT wrote, the opposite of a runtime owning a lifecycle.
- **D2 makes a declared LangGraph topology a Flow**, through the one new gate matcher `declared_pipeline`. The relation stays `call` at the schema level (widening `FlowRelation` would touch the renderer, the audit's E2 and the goldens for one framework); the precedent for an arrow whose source is not subject code is `process_launch`. Node bodies are deliberately NOT traced - that is the tracer's story, and on this subject it is a quarantine.

Both subjects are pinned per D5 (`test/fixtures/{data-science-agent,futures-trading-bot}.flow-producer.json`, regenerated by `scripts/measure-python-flow.ts`, asserted by `test/run/parity.test.ts`); ftb is private, so regenerating its fixture needs access, the same constraint the swe-prep fixture carries.
The number that matters most in those fixtures is `overturned_by_the_gate`, and it is **zero on both**: the producer and the gate fail closed on the same lines.
At the pinned SHAs 14 of the 23 routes verify, plus two program entries and the one declared topology, and both of report 5.1's hand-walked flagships verify at exactly the shape it predicted - `GET /decision-logs/{}` at eight boxes and narrative depth five, and the five-node topology at #39-`unknown`.

Out of v1 and named as such: the shared-state archetype for Python (dsa has the SQL and no container, ftb has neither and writes JSONL), the transport seam (neither subject has a first-party JS caller), the `justfile` launch reader, Typer/Click subcommand tables, runtime subscription registries, and cross-module type inference.
