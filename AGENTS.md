...
`src/probes/flow/` is the shared machinery and `src/probes/library/flow-{java,typescript}-*.ts` are the three registered adapters (#35, PRs 4-6). One family per adapter, deliberately: "no Spring here", "no runnable main here" and "no frontend calls anything here" are different findings, which is what the `Probe.applies` hook exists for - a framework-level applicability answer the toolchain test cannot give.
